...
import { updateBudget } from "@/lib/api"; // Import the updateBudget function

// Define a type for the budget data to ensure type safety
interface Budget {
  id: string;
  category: string;
  limit_amount: number;
...
interface EditBudgetModalProps {
  ...
  onClose: () => void;
  onSuccess: () => void;
  editingBudget: Budget; // The budget object to be edited
}
...
export default function EditBudgetModal({
  ...
  const [limit, setLimit] = useState(String(editingBudget.limit_amount));
  const [icon, setIcon] = useState(editingBudget.icon);
  const [color, setColor] = useState(editingBudget.color);
  const [description, setDescription] = useState(editingBudget.description);
  const [tags, setTags] = useState<string[]>(editingBudget.tags || []);
  const [loading, setLoading] = useState(false);

  // Use a useEffect hook to update state when editingBudget prop changes
  useEffect(() => {
    if (editingBudget) {
      setCategory(editingBudget.category);
      setLimit(String(editingBudget.limit_amount));
      setIcon(editingBudget.icon);
      setColor(editingBudget.color);
      setDescription(editingBudget.description);
      setTags(editingBudget.tags || []);
    }
  }, [editingBudget]);
    ...
    <Modal visible={visible} animationType="slide" transparent>
      <View className="flex-1 justify-center items-center bg-black/50 px-6">
        <View className="bg-white w-full p-6 rounded-lg">
          <Text className="text-xl font-medium mb-4">Edit Budget</Text>
          ...
          >
            <Text className="text-white font-medium">
              {loading ? <ActivityIndicator color="#fff" /> : "Update Budget"}
            </Text>
          </TouchableOpacity>
